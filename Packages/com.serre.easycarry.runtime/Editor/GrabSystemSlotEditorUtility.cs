...
{
    internal static class GrabSystemSlotEditorUtility
    {
        private const string PrefabRelativePathFormat = "Prefab/GrabSystem_{0:00}.prefab";
        private const string MenuSettingsNameFormat = "GrabItem_{0:00}_Settings";
        private const string MenuResetNameFormat = "GI_{0:00}_Reset";
        private const string MenuSwitchHandsNameFormat = "GI_{0:00}_SwitchHands_Enable";
        private const string MenuFreezeNameFormat = "GI_{0:00}_Freeze_Enable";
        ...
        {
            "GI_ItemSize", "AP_Contact_Hand_L", "AP_Contact_Hand_R", "AP_Contact_00", "AP_Contact_01",
        ...
        {
            "giItemSize", "apContactHandL", "apContactHandR", "apContact00", "apContact01",
        ...
        internal static void DrawSlotSelector(GrabSystemItemReference targets)
        {
            var activeSlot = ResolveSlot(targets);
            if (activeSlot >= 0 && targets.GISlot != activeSlot)
            {
                targets.SetGISlot(activeSlot);
        ...
        private static void DrawSlotButtons(GrabSystemItemReference targets, int activeSlot, bool[] usedSlots)
        {
        ...
        private static void DrawSlotButton(GrabSystemItemReference targets, int slot, int activeSlot, bool isUsed)
        {
            var isActive = slot == activeSlot;
            var style = GrabSystemEditorSharedUtility.CreateReadableStyle(EditorStyles.miniButton);
            if (isActive)
            {
        ...
        private static bool[] FindUsedSlots(GrabSystemItemReference current)
        {
            var usedSlots = new bool[SlotCount];
            var allTargets = Resources.FindObjectsOfTypeAll<GrabSystemItemReference>();
        ...
        private static int ResolveSlot(GrabSystemItemReference targets)
        {
            if (targets == null)
            {
                return -1;
            ...
            var prefabPath = targets.GeneratedGrabSystem != null
                ? PrefabUtility.GetPrefabAssetPathOfNearestInstanceRoot(targets.GeneratedGrabSystem)
                : string.Empty;
            for (var slot = 0; slot < SlotCount; slot++)
            {
                if (!string.IsNullOrEmpty(prefabPath) && prefabPath.EndsWith($"GrabSystem_{slot:00}.prefab"))
                {
                    return slot;
                }
            }

            return targets.GISlot >= 0 && targets.GISlot < SlotCount ? targets.GISlot : -1;
        }

        private static void ReplaceSlot(GrabSystemItemReference itemReference, int newSlot)
        {
            ...
            var prefabPath = GrabSystemAssetLocator.GetAssetPath(
            ...
            GrabSystemEditorSharedUtility.PrepareForSlotReplacement(itemReference);
            ...
            var newGIRoot = FindChildRecursive(newTransform, "GI_Root");
            if (newGIRoot == null)
            {
            ...
            GrabSystemEditorSharedUtility.SyncNumberedAttachPointAvailability(itemReference, false);
            RetargetItem(itemReference, newGIRoot);
        ...
        private static void RetargetItem(GrabSystemItemReference itemReference, Transform newGIRoot)
        {
#if VRC_SDK_VRCSDK3
            RetargetConstraint(itemReference.GetComponent<VRCParentConstraint>(), newGIRoot);
            RetargetConstraint(itemReference.GetComponent<VRCScaleConstraint>(), newGIRoot);
#endif
        }
        ...
        internal static void CaptureAndStoreSettings(GrabSystemItemReference targets)
        {
        ...
        internal static void ApplyStoredSettings(GrabSystemItemReference targets, bool restoreGIItemSizeScale)
        {
            if (targets == null || targets.GrabSystemRoot == null)
            {
                return;
            ...
            {
                RestoreSnapshot(targets, settings, restoreGIItemSizeScale);
            }
            else
            {
        ...
        internal static GrabSystemItemSettings CaptureSnapshot(GrabSystemItemReference targets)
        {
            if (targets == null || targets.GrabSystemRoot == null)
            {
                return targets != null ? targets.ItemSettings : new GrabSystemItemSettings();
            }

            GrabSystemEditorSharedUtility.EnsureMenuObjectReferences(targets);
            var snapshot = new GrabSystemItemSettings
            {
                Initialized = true,
                NumberedAttachPointOrder = targets.GetNumberedAttachPointOrder(),
                SourceSlot = Mathf.Clamp(targets.GISlot, 0, SlotCount - 1),
            ...
            for (var i = 0; i < AttachPointNames.Length; i++)
            {
                ...
                var attachPointTransform = FindChildRecursive(targets.GrabSystemRoot, attachPointName);
                var attachPointSnapshot = new GrabSystemAttachPointSettings
                {
            ...
            {
                snapshot.Contacts[i] = CaptureContact(targets.GrabSystemRoot, ContactNames[i]);
            }

            var mainConstraint = FindParentConstraint(FindChildRecursive(targets.GrabSystemRoot, "GI_MainConst"));
        ...
        internal static void RestoreSnapshot(GrabSystemItemReference targets, GrabSystemItemSettings snapshot,
            bool restoreGIItemSizeScale)
        {
            if (targets == null || targets.GrabSystemRoot == null || snapshot == null || !snapshot.Initialized)
            {
                return;
            }
            ...
            GrabSystemEditorSharedUtility.EnsureMenuObjectReferences(targets);
            var targetSlot = Mathf.Clamp(targets.GISlot, 0, SlotCount - 1);
            ...
            for (var i = 0; i < AttachPointNames.Length; i++)
            {
                ...
                ApplyAttachPoint(targets.GrabSystemRoot, AttachPointNames[i], attachPoint);
                RestoreBoneProxyAttachPoint(targets.GrabSystemRoot, AttachPointNames[i], attachPoint);
                RestoreAttachmentMethod(targets.GrabSystemRoot, AttachPointNames[i], attachPoint.AttachmentMethod);
            }

            for (var i = 0; i < ContactNames.Length; i++)
            {
                var contactTransform = FindChildRecursive(targets.GrabSystemRoot, ContactNames[i]);
                ...
                RestoreContact(contactTransform, snapshot.Contacts[i], i != 0 || restoreGIItemSizeScale);
            }
            ...
            {
                var serializedConstraint = new SerializedObject(mainConstraint);
                Undo.RecordObject(mainConstraint, "Restore GI_MainConst Weights");
                for (var i = 0; i < Mathf.Min(MaxSourceCount, snapshot.MainWeights.Length); i++)
                {
                    var weightProperty = serializedConstraint.FindProperty($"Sources.source{i}.Weight");
        ...
        private static void CaptureOptionDefaults(GrabSystemItemReference targets, GrabSystemItemSettings snapshot)
        {
            var parametersComponent = targets.GeneratedGrabSystem != null
        ...
        private static void RestoreOptionDefaults(GrabSystemItemReference targets, GrabSystemItemSettings snapshot)
        {
            var parametersComponent = targets.GeneratedGrabSystem != null
                ? targets.GeneratedGrabSystem.GetComponent<ModularAvatarParameters>()
                : null;
        ...
        private static void RestoreBoneProxyAttachPoint(Transform root, string attachPointName, GrabSystemAttachPointSettings snapshot)
        {
        ...
        private static void RestoreAttachmentMethod(Transform root, string attachPointName,
            GrabSystemAttachPointMethod attachmentMethod)
        {
            var attachPoint = FindChildRecursive(root, attachPointName);
            if (attachPoint == null)
            {
            ...
            var usesBoneProxy = attachPointName == "AP_Hand_L"
                ...
                || attachmentMethod == GrabSystemAttachPointMethod.BoneProxy;
        ...
        private static void ApplyAttachPoint(Transform root, string attachPointName, GrabSystemAttachPointSettings snapshot)
        {
            var attachPoint = FindChildRecursive(root, attachPointName);
        ...
        private static Transform GetAttachPointSource(GrabSystemItemReference targets, string attachPointName)
        {
        ...
        private static GrabSystemContactSettings CaptureContact(Transform root, string contactName)
        {
            ...
            var snapshot = new GrabSystemContactSettings
            {
                Initialized = true,
        ...
        private static void RestoreContact(Transform contactTransform, GrabSystemContactSettings snapshot, bool restoreScale)
        {
